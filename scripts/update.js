...
const margePackageJson = (json, dependencies) => {
  return ({...json, dependencies: {...json.dependencies, ...dependencies}});
};
const updatePackageJson = (res) =>
  fs.readFile('package.json', {encoding: 'utf8'})
    .then((json) => res.reduce((pre, cur) => margePackageJson(pre, cur.data.dependencies), JSON.parse(json)))
...
axios.get('https://api.github.com/repos/facebook/create-react-app/tags', {headers: {Accept: 'application/vnd.github.v3+json'}})
  ...
  .then(version => fs.readFile(VERSION_FILENAME, {encoding: 'utf8'})
    .then(data=> {
      if (data && version.version === data.match(/.+/)[0]) {
        ...
        return Promise.all([
          ...
          Promise.all([axios.get(`https://raw.githubusercontent.com/facebook/create-react-app/${version.version}/packages/cra-template-typescript/template.json`),
